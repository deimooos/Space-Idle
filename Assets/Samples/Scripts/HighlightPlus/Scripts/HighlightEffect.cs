...
namespace HighlightPlus {
    ...
    public partial class HighlightEffect : MonoBehaviour {
        ...
        private void Start()
        {
            startingGlow = this.glow;
        }

       /* private void Update()
        {
            if (PlayerMovement.scr != null)
            {
                
                if (Vector3.Distance(transform.position, Camera.main.transform.position) >= 125)
                {
                    this.glow = startingGlow * 0.01f;
                    this.outline = 0.01f;
                }
                else if (Vector3.Distance(transform.position, Camera.main.transform.position) > 25)
                {
                    this.glow = startingGlow * 75 / Vector3.Distance(transform.position, Camera.main.transform.position);
                    this.outline = 75/Vector3.Distance(transform.position, Camera.main.transform.position);
                    if (this.outline > 1)
                        this.outline = 1;
                }
                else
                {
                    this.glow = startingGlow;
                    this.outline = 1;
                }
            }
